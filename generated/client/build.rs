...
fn main() {
    ...
    let source = fs::read_to_string(&source_path).unwrap_or_else(|error| {
        panic!("cannot read database source {}: {error}", source_path.display())
    });
    ...
    for item in file.items {
        ...
        for item in item.items {
            ...
            if !method.attrs.iter().any(|attr| attr.path().is_ident("ipc")) { continue }
            methods.push(method);
        }
    }
    ...
    let output_path = manifest.join("src/generated_api.rs");
    ...
    if !unchanged {
        ...
        fs::write(&temporary_path, output).unwrap_or_else(|error| {
            panic!("cannot write generated client {}: {error}", output_path.display())
        });
...
fn ipc_options(method: &ImplItemFn) -> (Option<syn::Ident>, Option<syn::Ident>) {
    ...
    for attr in &method.attrs {
        if !attr.path().is_ident("ipc") { continue }
...
fn owned_type(ty: &Type) -> Type {
    ...
    } else if let Type::Path(path) = reference.elem.as_ref() && path.path.is_ident("str") {
...
fn pascal_case(name: &syn::Ident) -> syn::Ident {
    ...
    for c in name.to_string().chars() {
        if !c.is_ascii_alphanumeric() { upper = true; continue }
        if upper { output.extend(c.to_uppercase()); upper = false } else { output.push(c) }
    }
    format_ident!("{output}")
}

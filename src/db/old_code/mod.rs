...
use self::roaring::{InternalCacheType, RelationshipStorage};
use self::tag_search::TagSearchCache;
use crate::{Arc, DB_VERSION, plugins::PluginManager};
...
pub(crate) use super::SQL_CHUNK_SIZE;
...
",
    ...
    ///
    /// The recorded audit trail was removed; kept as a no-op so legacy call
    /// sites that threaded a `reason` string through the write path compile.
    ///

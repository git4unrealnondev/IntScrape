                ...
                for file in &files.nodes {
                    match file.kind {
                        NodeKind::File | NodeKind::Folder => {
                            ...
                            let Some((mut file_key, attrs)) = file_key.split('/').find_map(|key| {
                                ...
                                let attr_key = if file.kind.is_file() {
                                    let mut merged = file_key.clone();
                                    utils::unmerge_key_mac(&mut merged);
                                    merged[..16].try_into().ok()?
                                } else {
                                    file_key.as_slice().try_into().ok()?
                                };
                                let mut attr = BASE64_URL_SAFE_NO_PAD.decode(&file.attr).ok()?;
                                let attrs =
                                    NodeAttributes::decrypt_and_unpack(&attr_key, &mut attr)
                                        .ok()?;
                                Some((file_key, attrs))
                            }) else {
                                continue;
                            ...
                            let node = Node {
                                ...
                                children: nodes
                                    ...
                                    .filter_map(|it| {
                                        let parent = it.parent.as_ref()?;
                                        (parent == &file.handle).then(|| it.handle.clone())
                                    })
                                    .collect(),
                                aes_key,
                                aes_iv,
    ...
    pub async fn download_node<W: AsyncWrite>(&self, node: &Node, writer: W) -> Result<()> {
        let responses = if let Some(download_id) = node.download_id() {
            let request = if node.handle.as_str() == download_id {
                Request::Download {
                    g: 1,
                    ssl: if self.state.https { 1 } else { 0 },
                    n: None,
